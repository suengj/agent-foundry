...
def _intersect_write_scopes(role_scopes: list[str], work_item_scopes: list[str]) -> list[str]:
    if not role_scopes:
        return sorted(work_item_scopes)
    if not work_item_scopes:
        return sorted(role_scopes)
    role_set = {scope.rstrip("/") for scope in role_scopes}
    work_set = {scope.rstrip("/") for scope in work_item_scopes}
    matched = sorted(role_set & work_set)
    if matched:
        return matched
    prefix_matches: list[str] = []
    for work_scope in sorted(work_set):
        for role_scope in sorted(role_set):
            if work_scope.startswith(role_scope) or role_scope.startswith(work_scope):
                prefix_matches.append(work_scope)
    if prefix_matches:

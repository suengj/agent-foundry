...
- **Tier A — known vendor credential formats** (OpenAI-style `sk-` keys, with or without a label from the closed set `live`, `test`, `proj`, `svcacct`, `admin`; GitHub tokens; Slack tokens; AWS access keys; Google API keys; GitLab/npm/Doppler/Stripe prefixes; PEM/PGP private-key armor; JWTs whose JOSE header carries an `alg` member in any position) in string **values** or dict **keys** cause a hard failure. Strong prefixes match on format alone. The weak `sk-` prefix collides with ordinary language (`risk-`, `task-`, `mask-`), so it is discriminated structurally: an unbroken alphanumeric body of 16+, or — since real project keys are base64url and may contain `-` — a hyphenated body that is 24+ characters, keeps hyphens under one in ten, and carries both a digit and an upper-case character. Real keys are long with sparse hyphens; vocabulary is short with a hyphen every few characters, so `sk-live-Feature-Toggle-2024` and `sk-live-feature-toggle-enabled` stay writable while `sk-proj-<base64url>` does not. `SecretRef` mappings are traversed field-by-field; credential-shaped values in `name`, `scope`, or `version` are not exempt.
...
- Detection runs at the serialization boundary, **not** in model validation. A false positive on Tier A is recoverable: the project can still be loaded and inspected; only the write is refused, with a diagnostic naming the JSON path and the rule that matched. The diagnostic never carries the secret value; the excerpt is a 4-character prefix plus a length, which for Tier A is a format constant (`ghp_`, `AKIA`).
- A per-path `allow_paths` escape exists for legitimate values that trip Tier A. It is per-path by design, not a global off-switch. Paths are matched structurally: `adapter_options.a.b` means nested keys `a` then `b`, not a literal key `a.b`. Escape a literal dot in a key name with `\\.` (for example `adapter_options.a\\.b`). The bracket form `[key]` marks a credential used as a dict **key** (distinct from a literal key named `@key`, which encodes as `y.@key`). Diagnostic messages use the escaped dotted form, so a reported path can be pasted into `allow_paths` — except where a segment is redacted, as described next. Non-string mapping keys are rendered as the string the JSON document actually emits (`null`, `true`, `1.5`), which keeps the reported path addressable and stops an integer key from colliding with a list index.

...
- **Tier A — known vendor credential formats** (OpenAI-style `sk-` keys with a closed label set — `live`, `test`, `proj`, `svcacct`, `admin` — and an unbroken alphanumeric body; GitHub tokens; Slack tokens; AWS access keys; Google API keys; GitLab/npm/Doppler/Stripe prefixes; PEM private-key headers; JWTs with a valid `{"alg"` header) in string **values** or dict **keys** cause a hard failure. Strong prefixes match on format alone; the weak `sk-` prefix requires a single unbroken alphanumeric body after an optional known label (not hyphenated dictionary words). `SecretRef` mappings are traversed field-by-field; credential-shaped values in `name`, `scope`, or `version` are not exempt.
